...
- 当 `R/C ≥ M/2`，最小值出现在 `k=M/2`，即**均分**到两个处理器最优。
...
其中只能计不同任务间的成对通信。当 `R/C ≥ M/2` 且 `N` 很大、`M` 是 `N` 的整数倍时，比较 `N` 处理器与单处理器的总时间（第 43–44 页），可得 speed-up：

`S = (RN/C) / (R/C + M(N−1)/2)`

若 `R/C >> M(N−1)/2`，则 speed-up 近似随 `N` 线性增长；否则第二项占主导，speed-up 渐近趋于 `R/(C·M)`，与处理器数无关——此时继续加处理器无收益（第 44 页）。

讲义还用 `Tdiff = [R·M/N + C·M²/2 − C·M²/(2N)] − R·M` 比较 `N` 处理器与单处理器，并令 `Tdiff=0` 解出 `R/C = M/2`（第 43 页），这正是两处理器结论的推广。

## 重叠模型的乐观情形

完全重叠时，通信被计算掩盖，执行时间取计算项与通信项的最大值而不是相加（第 45 页）：

`T = max{ R·max_i k_i, (C/2)·Σ_i k_i(M−k_i) }`

讲义还提出一个变体：若通信开销与处理器数成正比（`T = R·max k_i + C·N`），则应分析“加更多处理器”的效果（第 46 页）。这个差异体现了“多个工人同时工作”和“必须等交接完成”两种不同现场。

## 一个数值例题

设 `M=100`、`R=2`、`C=1`，则 `R/C=2 < M/2=50`，按讲义应在单个处理器上执行全部任务。此时无跨处理器通信，总时间约 `R·M=200`；若勉强均分（`k≈50`），计算项为 `2·50=100`、通信项为 `1·50·50=2500`，总时间约 `2600`，远大于单处理器。反过来，设 `R=60`、`C=1`，则 `R/C=60 > M/2=50`，均分更优：计算项 `60·50=3000`、通信项 `2500`，总时间约 `5500`，仍低于单处理器 `6000`。可见阈值 `R/C = M/2` 决定“是否值得拆任务”，而不是只看处理器数量。
