...
图 1：CLOS 三级网络（输入级 r1×r2、中间级 m、输出级 m×n2，任意输入有路径到任意输出）；来源：[Chap 2_2_MINs.pdf](https://github.com/Kytolly/NUS-coursework-wiki/releases/download/CEG5201/Chap%202_2_MINs.pdf) 第 6 页；核对 2026-09-07。
...
CLOS 的参数一般记作：输入级每个模块为 `r1×r2` 交换，最后一级为 `m×n2` 交换，中间级有 `m` 个模块；`n1`、`n2` 是输入/输出模块的端口数，`m` 是中间级模块数。增加 `m` 或每个模块的可用链路会提升路径多样性，但代价是交换单元、布线与控制复杂度上升。
...
其中 `n1`、`n2` 是输入/输出模块的端口数（讲义也写作中间级元素数需足够多）。这就是 CLOS 之所以被选用的原因：高可扩展性（连接数千个 PE/加速器）、高带宽、低时延（多路径降低拥塞）、成本效益（比全 crossbar 用更少交换机）。
...
> **Question:** 课堂采用的 CLOS 阻塞证明步骤与 switch-state convention 是什么？`m ≥ n2 + n1 − 1` 的证明是否按讲义定义展开？
...
- 2026-09-07：扩写三种性质、CLOS 判据 `m ≥ n2+n1−1` 与 Fat-tree 关系，新增 CLOS 原图（第 6 页）并补 Open 问题。
