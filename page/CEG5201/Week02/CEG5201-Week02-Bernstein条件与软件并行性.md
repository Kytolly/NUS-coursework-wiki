...
对两个过程 $P_i$ 和 $P_j$，令 $I_i / I_j$ 为输入集合、$O_i / O_j$ 为输出集合。两者可并行（记为 $P_i \parallel P_j$）当且仅当（[CG5201_Chap2 (2627).pdf](https://github.com/Kytolly/NUS-coursework-wiki/releases/download/CEG5201/CG5201_Chap2%20%282627%29.pdf) 第 14–15 页）：
...
三式分别排除 **flow（RAW）**、**anti（WAR）** 和 **output（WAW）** 依赖。对 $n$ 个过程，需要两两检查 $\frac{3n(n-1)}{2}$ 个关系；只要有一对不满足，就禁止这部分并行（第 16 页）。

$\parallel$ 关系有以下性质（第 17 页）：通常是**交换**的，**不传递**（例如 $P_1 \parallel P_5$ 且 $P_5 \parallel P_2$，但 $P_1$ 与 $P_2$ 可能不独立），因此**不是等价关系**；结合律成立；$I_i \cap I_j \ne \emptyset$（只读共享）**并不**阻止并行。
...
设 $P_1$ 读集合 $\{a,b\}$、写集合 $\{c\}$；$P_2$ 读集合 $\{c,d\}$、写集合 $\{e\}$。检查三式：$I_1 \cap O_2 = \emptyset$，但 $I_2 \cap O_1 = \{c\} \ne \emptyset$，存在 anti/WAR 相关，因此 $P_1$ 与 $P_2$ 不能并行。若把 $P_2$ 改为读 $\{d\}$、写 $\{e\}$，三式均为空，则 $P_1 \parallel P_2$ 成立。

对 $n=3$ 个过程，需两两检查 $\frac{3 \times 3 \times 2}{2} = 9$ 对集合关系；只要有一对不满足，就禁止相应子集并行（第 16 页）。因此实际判定时，先把每个过程的输入/输出集合列全，再去核对交集，避免凭感觉判断。

$\parallel$ 不传递也意味着不能简单做传递闭包：讲义给出 $P_1 \parallel P_5$ 且 $P_5 \parallel P_2$，但 $P_1$ 与 $P_2$ 可能因共享某个输出变量而不独立（第 17 页）。所以在分析整段程序时，必须成对检查而不仅是相邻对。
...
$\parallel$ 不传递意味着“每个相邻对都满足”并不能推出“整体可并行”；只读共享（输入集合相交）本身不禁止并行。判定时必须同时核对输入/输出集合与资源依赖。
