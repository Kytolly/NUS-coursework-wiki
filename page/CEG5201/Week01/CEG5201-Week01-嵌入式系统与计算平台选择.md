...
$$
P = \frac{1}{2} f C V_{dd}^2
$$

其中 $f$ 是频率、$C$ 是开关电容、$V_{dd}$ 是电源电压。讲义还给出 $f = \frac{k(V_{dd}-V_T)^2}{V_{dd}}$（$V_T$ 为阈值电压），因此 $P$ 对 $V_{dd}$ 是二次甚至更高阶依赖，$P$ 作为 $f$ 的函数是凸函数。于是出现 **Dynamic Voltage Scaling (DVS)** 处理器：用一组电压档控制频率，把“截止时间”与“功耗”变成联合优化，即算法要在时间与功耗预算之间折中。
