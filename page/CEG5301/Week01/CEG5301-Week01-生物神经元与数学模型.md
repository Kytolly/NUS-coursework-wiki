...
$$
y = \varphi\left( \sum_j w_j x_j - b \right)
$$
...
$$
\begin{aligned}
u_k &= \sum_j w_{kj} x_j \quad &\text{（线性组合器输出）} \\
v_k &= u_k + b_k \quad &\text{（诱导局部场）} \\
y_k &= \varphi(v_k) \quad &\text{（神经元输出）}
\end{aligned}
$$
...
| 部分 | 数学量 | 作用 |
|---|---|---|
| 突触/连接 | 权重 $w_{kj}$ | 表示兴奋（正）或抑制（负）；负权可抑制放电 |
| 加法器（线性组合器） | $u_k = \sum_j w_{kj} x_j$ | 对输入信号加权求和 |
| 激活函数 | $\varphi(\cdot)$ | 限制输出幅值，如 $[0,1]$ 或 $[-1,1]$（One 第 44 页） |
...
令诱导局部场采用 $v = w_1 x_1 + w_2 x_2 + b$，阈值在 0，激活取阶跃 $\varphi(v)=1\ (v \ge 0)\text{，否则 } 0$。取 $w_1=0.5$、$w_2=-0.3$（负权表示抑制）、$b=0.2$：

- 输入 $x=(1,1)$：$u = 0.5 \times 1 + (-0.3) \times 1 = 0.2$，$v = u + b = 0.4 \implies y = 1$（放电）。
- 输入 $x=(0,1)$：$u = -0.3$，$v = -0.1 \implies y = 0$（不放电）。
- 输入 $x=(1,0)$：$u = 0.5$，$v = 0.7 \implies y = 1$。

可见负权重能把本来“正的求和”拉低到阈值以下，从而**抑制**神经元放电；这正是只用 $y=\sum x_i$、输入不能为负时所无法建模的（One 第 40–43 页）。
