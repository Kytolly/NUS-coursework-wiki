...
SOM 是一个**二维神经元阵列**（lattice）。每个神经元 j 有权重向量 `w_j = [w_j1, …, w_jn]`，并与网格中所有神经元有加权连接；输入向量 `x = [x1, …, xn]` 连接到所有神经元。

类比：像一张地图，每个格子是一个神经元，权重就是它“记住”的输入。

## 算法概述

① 随机初始化所有权重；② 从训练集选一个输入 `x`；③ 把 `x` 与各神经元权重 `w_j` 比较，确定**胜者**；④ 更新胜者及其邻域神经元，使它们更像 `x`；⑤ 调整学习率与邻域函数；⑥ 重复 ②—⑤ 直到收敛（权重不再明显变化）或达到训练轮数。

## SOM 形成的三个过程

1. **竞争（Competition）**：连续输入空间被映射到离散输出空间，找**最佳匹配神经元**：
   `i(x) = argmin_j ‖x − w_j‖`（欧氏距离最小者获胜）。
2. **合作（Cooperation）**：胜者确定一个**拓扑邻域**中心；神经生物学证据表明，越靠近胜者的神经元受激发越强，因此用随距离衰减的**邻域函数 h(n)** 描述相互影响。
3. **自适应（Weight Adaptation）**：
   `Δw_j(n) = η(n) h_{j,i(x)}(n) (x − w_j(n))`
   胜者权重向量向 `x` 移动；邻域内神经元也向 `x` 移动，**越远变化越小**。
...
1. 随机初始化各神经元权重 `w_i`；
2. **采样**：从训练集选择输入 `x`；
3. 确定胜者 `k`：`‖w_k − x‖ = min_i ‖w_i − x‖`；
4. 更新胜者邻域内所有权重：`w_j(n+1) = w_j(n) + η(n) h_{j,k}(n) (x − w_j(n))`；
5. 满足收敛判据则停止，否则回到 2。
...
课件以 2 维输入（客户年龄与收入）为例：`X=[30, 50000]`。初始化一个 10×10 的 SOM（100 个神经元），每个神经元 j 有 2 维权向量，如 `W₁=[25,65000]`、`W₂=[45,40000]`、…、`W₁₀₀=[35,60000]`（Four 第 57 页）。

**Step 1 找 BMU**：对每个神经元 j 计算 `d_j = ‖X − W_j‖`，距离最小者（设为神经元 37）即胜者。

**Step 2 更新**：`W_j^new = W_j^old + η(t)·h(i,BMU,t)·(X − W_j^old)`，其中 `η(t)` 为学习率、`h` 为邻域函数（通常高斯）、`X − W_j^old` 为输入与权重的差。胜者与邻域神经元都朝 X 移动，越远变化越小。
...
> **Open**：课件未给出邻域函数 `h(t)` 与学习率 `η(t)` 的具体衰减公式，仅说明常用高斯邻域与随机/序列采样，因此不展开。
