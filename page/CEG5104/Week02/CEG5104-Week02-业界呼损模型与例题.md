...
```
p_b = e^{−A} · Σ_{i=N+1}^{∞} A^i / i!
```

（A = 提供话务，N = 中继线/信道数。来源：2_capacity_analysis.pdf 第 63 页。）

**Erlang-C 公式（BCD）**：无限独立信源、丢失呼叫被“延迟”、服务时间指数、按到达顺序服务：

```
C(N, A) = (A^N/N! · 1/(1−A/N)) / (Σ_{i=0}^{N−1} A^i/i! + A^N/N! · 1/(1−A/N))
```
...
- **例 1（Erlang 话务与 MSC 数）**：总人口 20 万、渗透率 25% → 5 万用户。移动→固定/固定→移动：保持 100 s、3 次/时；移动→移动：保持 80 s、4 次/时；话务占比 50% / 40% / 10%。做法：每类话务强度 = 呼叫率 × 保持时间（小时），按占比加权后再乘用户数得总 Erlang；每个 MSC 承载 1800 Erl，所需 MSC = ⌈总 Erlang / 1800⌉。

  计算（重建）：每用户话务 = 3×(100/3600)×0.50 + 3×(100/3600)×0.40 + 4×(80/3600)×0.10 ≈ 0.0839 Erl ⇒ 50000×0.0839 ≈ 4194 Erl ⇒ MSC = ⌈4194/1800⌉ = 3 个。（课件只给数据与思路，未给最终值；此值为按上述方法重建。）

- **例 2（M/M/3/6 稳态）**：λ = 40/min、μ = 30/min，c = 3、K = 6，ρ = λ/(cμ) = 40/90 = 4/9。画状态转移图，解 M/M/c/K 平衡方程得 p₀…p₆；提供话务 a = λ/μ = 4/3 Erl；**有效话务** = 提供话务 × (1 − 呼损)，**阻塞话务** = 提供话务 × 呼损。（来源：2_capacity_analysis.pdf 第 66 页。）

- **例 3（BS-BSC MMS 队列）**：基站为 MMS 预留 3 信道，忙时最多再等 4 条，故为 M/M/3/7 有限容量队列；峰值 λ = 1 条/s、平均服务 6 s。用 M/M/c/K 稳态求系统内平均消息数、平均等待时间（Little 定律 L = λW）与丢失比例。（来源：2_capacity_analysis.pdf 第 67 页。）
